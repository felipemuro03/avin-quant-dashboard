...
st.title("Analise Tecnica e Oportunidades ETFs")
st.caption(
...
def render_universo_precos():
    ...
    categorias_selecionadas = _filtro_categorias(key="cat_universo")
    st.caption(
    ...
    periodo_grafico = st.radio(
        "Ver variacao de:", colunas_variacao, horizontal=True, index=1, key="periodo_grafico_universo"
    )
...
def render_correlacoes():
    ...
    with col_f1:
        categorias_selecionadas = _filtro_categorias(key="cat_corr")
    with col_f2:
        anos_janela_corr = st.slider("Janela de correlacao (anos)", 1, 10, 3, key="janela_corr")
    with col_f3:
        anos_janela_zscore = st.slider("Janela de valor relativo / z-score (anos)", 1, 15, 5, key="janela_zscore_corr")
...
def render_analise_tecnica():
    ...
    categorias_selecionadas = _filtro_categorias(key="cat_tecnica")
    universo = universo_base[universo_base["Categoria"].isin(categorias_selecionadas)].copy()
    ...
    with st.expander("Parametros avancados"):
        periodo_rsi = st.slider("Periodo do RSI", 5, 30, 14, key="periodo_rsi_tecnica")
        janela_vol_curta = st.slider("Janela da volatilidade recente (dias)", 10, 60, 20, key="janela_vol_curta_tecnica")
        anos_vol_historico = st.slider("Janela historica p/ regime de vol (anos)", 1, 10, 3, key="anos_vol_historico_tecnica")
        janela_bollinger = st.slider("Janela das Bandas de Bollinger (dias)", 10, 60, 20, key="janela_bollinger_tecnica")
        desvios_bollinger = st.slider("Bandas de Bollinger (desvios-padrao)", 1.0, 3.0, 2.0, step=0.5, key="desvios_bollinger_tecnica")
    ...
    ticker_selecionado = st.selectbox(
        ...
        key="ticker_selecionado_tecnica",
    )
...
def render_oportunidades():
    ...
    col_f1, col_f2, col_f3 = st.columns([2, 1, 1])
    with col_f1:
        categorias_selecionadas = _filtro_categorias(key="cat_oport")
    with col_f2:
        anos_valor_relativo = st.slider("Janela de valor relativo (anos)", 1, 15, 5, key="janela_valor_relativo_oport")
    with col_f3:
        periodo_rsi = st.slider("Periodo do RSI", 5, 30, 14, key="periodo_rsi_oport")
